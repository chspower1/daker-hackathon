...
export function EmptyState({ title, description, actionLabel, onAction, icon, className }: EmptyStateProps) {
  return (
    <div className={cn("flex flex-col items-center justify-center p-8 text-center bg-white shadow-sm border border-slate-200 rounded-xl transition-all duration-300", className)}>
      {icon && (
