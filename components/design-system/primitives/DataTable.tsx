...
export function DataTable({ className, ...props }: DataTableProps) {
  return (
    <div className="w-full overflow-auto rounded-md border border-border-base bg-surface-base">
      <table
...
>(({ className, ...props }, ref) => (
  <tfoot
    ref={ref}
    className={cn("border-t border-border-base bg-surface-muted font-medium", className)}
    {...props}
  />
));
...
>(({ className, ...props }, ref) => (
  <tr
    ref={ref}
    className={cn(
      "border-b border-border-base transition-colors hover:bg-surface-subtle data-[state=selected]:bg-primary-subtle",
      className
    )}
    {...props}
  />
