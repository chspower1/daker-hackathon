...
이전의 단일 대형 명세(`development-spec.md`)는 공통/기능 문서 구조로 이전되었으며, 내용 중복을 제거했다. 병렬 작업이 목적이면 공통/기능 문서보다 `workstreams` 문서를 먼저 읽는다.
...
- 병렬 작업 시작점: `docs/workstreams/parallel-overview.md` + 해당 트랙 문서(`camp`, `hackathon`, `ranking`)
