...
- `lib/i18n`: 지원 언어 목록, locale dictionary, provider, locale 영속화 처리
...
- UI 문자열은 컴포넌트에 직접 하드코딩하지 않고 locale dictionary에서 읽는다.
- 초기 locale 결정은 쿠키를 기준으로 하고, `localStorage`는 클라이언트 편의용 미러로만 사용한다.
- 언어 전환이 가능한 페이지의 메타데이터 동기화 규칙과 신규 언어 추가 절차는 `docs/i18n-guide.md`를 단일 출처로 사용한다.
