...
4. 각 화면은 `lib/storage/entities/*` 또는 `lib/data/*` 를 통해 데이터를 읽고, 필요한 경우 시드 데이터를 fallback 으로 사용한다.
...
- 초기 locale 은 쿠키 기준으로 결정하고 `localStorage` 는 클라이언트 미러로만 사용한다.
...
- 라우트와 컴포넌트는 `window.localStorage` 를 직접 사용하지 않고 `lib/storage` 경계를 통해 접근한다.
